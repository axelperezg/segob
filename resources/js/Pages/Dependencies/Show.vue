...
import DependencyPresenter from '@/Presenters/DependencyPresenter';
...
const dependencyPresenter = new DependencyPresenter(props.dependency.data);
...
const throttledSearch = throttle((value) => {
    ...
    router.get(route('dependencies.show', props.dependency.data.slug), queryString, {
        preserveState: true,
        preserveScroll: true,
    });
}, 500);
...
let clearFilters = () => {
    router.get(route('dependencies.show', props.dependency.data.slug), {}, {
        preserveScroll: true,
    });
}
...
<template>
    <div>
        ...
        <div class="flex justify-center bg-[#F1EDE1] -mx-4 -mt-8 py-6 mb-8">
            <img :src="dependencyPresenter.banner" :alt="dependencyPresenter.name" class="w-[20rem]">
        </div>
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
                ...
                <!-- Right content -->
                <div>
                    <h1 class="text-3xl font-medium mb-8">{{ dependencyPresenter.name }}</h1>
                    <div class="space-y-6">
                        <div v-if="!posts.data.length" class="text-gray-500 text-center py-8 bg-gray-50 rounded-lg">
                            No hay publicaciones disponibles para esta dependencia
                        </div>
